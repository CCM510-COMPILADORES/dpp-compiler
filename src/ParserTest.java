...
public class ParserTest {
    ...
    private static void testarFor() {
        ...
        test("for com corpo vazio",             "style bloomfield(space x = 0; x < 10; x = x + 1;) { } borderline",                true);
    ...
    private static void testarFuncao() {
        ...
        test("função sem parâmetros",           "space PREY minhaFuncao[] { catapult(x); HOMETOWN } borderline",                            true);
        test("função com um parâmetro",         "space PREY minhaFuncao[space x] { catapult(x); HOMETOWN } borderline",                     true);
        test("função com dois parâmetros",      "space PREY minhaFuncao[space x, lithium y] { catapult(x); HOMETOWN } borderline",          true);
        test("função com retorno",              "space PREY minhaFuncao[space x] { HOMETOWN x } borderline",                                true);
        test("função com corpo vazio",          "space PREY minhaFuncao[] { HOMETOWN } borderline",                                         true);
        test("função sem HOMETOWN",             "space PREY minhaFuncao[] { catapult(x); } borderline",                                     false);
        test("função sem {",                    "space PREY minhaFuncao[] catapult(x); HOMETOWN borderline",                                false);
        test("função sem ]",                    "space PREY minhaFuncao[ catapult(x); HOMETOWN } borderline",                               false);
    }
}